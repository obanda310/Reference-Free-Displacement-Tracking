...
function [imgNBds6] = imageNoiseBounds(r,m3,image,shear,scale,noiseCutoff)
imgNBds = zeros(size(image.Black,1),size(image.Black,2));

xyStd = std2([shear.ltdXY(:,shear.noCellTraj) -1*shear.ltdXY(:,shear.noCellTraj)]);
xyVals = squeeze(shear.ltdXY(:,shear.noCellTraj));
xyVals = cat(1,xyVals,-1*xyVals);
xyVals(xyVals == 0) = NaN;
xyStd = std(xyVals(:),'omitnan');
xyStd2 = 2*xyStd;
...
for i = 1:size(r,1)
    try
        if (abs(m3.disp(i,3)) > noiseCutoff || dispXY(i,1) > xyStd2) && round(m3.refSC(i,2)/scale)<=size(imgNBds,1) && round(m3.refSC(i,1)/scale)<=size(imgNBds,2)
