...
for m = 1:size(goodData,1)
    try
        ...
        disp(dirList{goodData(m,1),1})
        cd(dirList{goodData(m,1),1});
        load('TractionOutputs.mat')
        compTi(1:500,1:500,m) = imresize(tiPN{1}{1},[500 500],'nearest');
        compTi(501:1000,1:500,m) = imresize(tiPN{1}{2},[500 500],'nearest');
        tFig = figure
        hold on
        imshow(compTi(:,:,m),[])
        t = text(.05,50,num2str(m),'FontSize',40,'FontWeight','bold','Color','white');
        tFigImage = getframe(gca,[0 100 280 560]);
        compT(:,:,m) = rgb2gray(tFigImage.cdata) ; 
%         figure
%         imshow(tFigImage.cdata,[])
    catch
        disp('failed')
        m
    end
...
compT(isnan(compT)) = 0;
compT = double(compT);
compT2 = (compT+(-1*min(compT(:))));
compT2 = (compT2/max(compT2(:)))*256;
for i = 1:size(compT,3)
    clear tempImage
    tempImage(:,:) = uint8(compT2(:,:,i));    
    imwrite(tempImage,[ListPath prefix 'CompTractions.tif'],'WriteMode','append');
end
%% Make Montage Figures
goodData = [1:1:size(dirList,1)]';
close all
clear temp temp2 shearMontage h hr hi
for m = 1:size(goodData,1)
%     try       
        disp(dirList{goodData(m,1),1})
        cd(dirList{goodData(m,1),1});
        if m == 1
        shearMontage = imread('HeatMaps\Traction\ShearTractions.tif');        
        h = size(shearMontage,1);
        else
        temp = imread('HeatMaps\Traction\ShearTractions.tif');
        hi = size(temp,1);
        hr = h/hi;
        temp2 = imresize(temp,[h round(hr*size(temp,2))]);
        shearMontage = cat(2,shearMontage,temp2);            
        end
%     catch
%     end
end
...
imshow(shearMontage)
    ...
    export_fig(shear,savefile,'-native');

clear temp temp2 normMontage h hr hi
for m = 1:size(goodData,1)
%     try       
        disp(dirList{goodData(m,1),1})
        cd(dirList{goodData(m,1),1});
        if m == 1
        normMontage = imread('HeatMaps\Traction\Ztractions.tif');        
        h = size(normMontage,1);
        else
        temp = imread('HeatMaps\Traction\Ztractions.tif');
        hi = size(temp,1);
        hr = h/hi;
        temp2 = imresize(temp,[h round(hr*size(temp,2))]);
        normMontage = cat(2,normMontage,temp2);            
        end
%     catch
%     end
end
...
imshow(normMontage)
    ...
    title = ['Normal Montage'];
    savefile = [ListPath title];
    export_fig(normal,savefile,'-native');
...
%% Determine Aspect Ratio of Cells

for i = 1:size(dirList,1)
    clear totals
    AR = cellAspectRatio(dirList{i});
    fAR(i,1:3) = AR;
    i
end

save([ListPath prefix 'AspectRatios.mat'],'fAR')
