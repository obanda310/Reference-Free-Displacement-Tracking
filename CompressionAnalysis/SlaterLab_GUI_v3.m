...
%%
    function Recalculate_Callback(~,~)
        ...
        data.iSampleHeight = get(hEstimatedInitialHeight,'string');
        data.iSampleHeight = str2double(data.iSampleHeight);
        if isempty(data.iSampleHeight) == 1
            msgbox('Error! Please input initial sample height!')
        else
        iSampleHeight = data.iSampleHeight; 
        [~,minLoadIdx] = min(abs(((data.extension*-1)+ data.iHeight)- iSampleHeight));
        end
