...
xdispdist = figure;
hold on
%histogram(book1(3,:,:)*.1625,50)
xStd = std2(book1(13,:,:)*.1625)
xVals = squeeze(book1(13,:,:));
xVals(xVals == 0) = NaN;
xStd = std(xVals(:)*.1625,'omitnan')
histogram(book1(13,:,:)*.1625,bins,'FaceColor',[.6 .6 .6],'Normalization','probability')
histmax = max(histcounts(book1(13,:,:)*.1625,bins,'Normalization','probability'));
...
yt = 'Relative Probability'; %input('enter the yaxis label','s');
...
axis([-0.4 0.4 0 round(histmax,2)+.01])
...
ydispdist = figure;
hold on
%histogram(book1(4,:,:)*.1625,50)
yVals = squeeze(book1(14,:,:));
yVals(yVals == 0) = NaN;
yStd = std2(book1(14,:,:)*.1625)
yStd = std(yVals(:)*.1625,'omitnan')
histogram(book1(14,:,:)*.1625,bins ,'FaceColor',[.6 .6 .6],'Normalization','probability')
histmax = max(histcounts(book1(14,:,:)*.1625,bins,'Normalization','probability'));
...
set(gca,'fontsize',AxisFontSize)
xt = 'Y-Displacement (\mum)';% input('enter the xaxis label','s');
yt = 'Relative Probability'; %input('enter the yaxis label','s');
tt = 'Line-Profile Displacements';%input('enter the title','s');
le = '\sigma'; %input('enter the legend','s');
le2 = '2*\sigma';
...
hold on
%histogram(book1(4,:,:)*.1625,50)
xyStd = std2([book1(15,:,:)*.1625 -1*book1(15,:,:)*.1625])
xyVals = squeeze(book1(15,:,:));
xyVals = cat(1,xyVals,-1*xyVals);
xyVals(xyVals == 0) = NaN;
xyStd = std(xyVals(:)*.1625,'omitnan')*1000
xyStd2 = 2*xyStd;
histogram(book1(15,:,:)*.1625*1000,bins ,'FaceColor',[.6 .6 .6],'Normalization','probability')
histmax = max(histcounts(book1(15,:,:)*.1625*1000,bins,'Normalization','probability'));
