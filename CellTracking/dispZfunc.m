...
%%
for i = 1:size(planesFinal,2)
    xyRef = cat(2,rXY(:,[2 1]),ones(size(rXY,1),1));
    zRef = cat(2,rRef2a(planesFinal(1:nnz(planesFinal(:,i)),i),[1 2]),2*ones(nnz(planesFinal(:,i)),1));
    xyzRef = cat(1,xyRef,zRef);
    lub{i} = trackmem(xyzRef,1.5,2,1,1);
end
%%
close all
figure
...
for i = 1:max(lub{j}(:,4))
    lubR = find(lub{j}(:,4)==i);
    plot3(lub{j}(lubR,1),lub{j}(lubR,2),lub{j}(lubR,3))
end
end
hold off
%%
rDisp2 = r(:,1:3)-rRef2a(:,1:3);
...
%hold on
quiver3(rRef2(:,1),rRef2(:,2),rRef2(:,3),rDisp2(:,1),rDisp2(:,2),rDisp2(:,3),0)
...
for i = 1:size(rowsLines,3)
    if nnz(rowsNDCU(i,:))>0
        ...
        for j = 1:size(xy,1)
            rRef1(rows(i,j),1:3) = xy(j,1:3);
        end
    else
        for j = 1:nnz(rows(i,:))
            rRef1(rows(i,j),1:3) = r(rows(i,j),1:3);
        end
    end
end
rDisp1 = r(:,1:3)-rRef1(:,1:3);
...
%% Quiver Plot
figure
quiver3(rRef1(:,1),rRef1(:,2),rRef1(:,3),rDisp1(:,1),rDisp1(:,2),rDisp1(:,3),0)
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i = 1:size(rowFits3,1)
    if nnz(rowsNDCU(i,:))>0
        ...
        [xy,distance,t_a] = distance2curve(temp,r(rows(i,1:n),1:3));
        for j = 1:size(xy,1)
            rRef3(rows(i,j),1:3) = xy(j,1:3);
        end
    else
        for j = 1:nnz(rows(i,:))
            rRef3(rows(i,j),1:3) = r(rows(i,j),1:3);
        end
    end
end
rDisp3 = r(:,1:3)-rRef3(:,1:3);
...
%% Quiver Plot of Displacements Method 3
 figure
 quiver3(rRef3(:,1),(size(imageBinary,1)*xyScale)-rRef3(:,2),rRef3(:,3),rDisp3(:,1),rDisp3(:,2),rDisp3(:,3),0)
