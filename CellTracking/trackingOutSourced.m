...
for l = 1:size(problems,1)
    ...
    nSize = 30; % max neighbor distance
    ...
    scatter3(fSizeYmin,fSizeXmin,lub(pPilIdx,6))
    ...
    for i = 1:size(nghbrs,1)
        if size(find(lub(:,7)==nghbrs(i,1) & lub(:,6)<lub(pPilIdx,6),1,'last'),1)>0
            ...
            scatter3(lub(nghbrs(i,2),1)-(cntrPt(1,1)-fSizeYmin),lub(nghbrs(i,2),2)-(cntrPt(1,2)-fSizeXmin),lub(nghbrs(i,2),6),'r')
            trckNum = num2str(nghbrs(i,1));
            tempInd1 = find(lub(:,7)==nghbrs(i,1),1,'first');
            tempInd2 = find(lub(:,7)==nghbrs(i,1),1,'last');
            if((tempInd2-tempInd1)>0)
                tempPillar = lub(tempInd1:tempInd2,:);
                plot3(tempPillar(:,1)-(cntrPt(1,1)-fSizeYmin),tempPillar(:,2)-(cntrPt(1,2)-fSizeXmin),tempPillar(:,6))
                %plot3(tempPillar(:,24),tempPillar(:,25),tempPillar(:,6))
            end
            ...
            text(lub(nghbrs(i,2),1)-(cntrPt(1,1)-fSizeYmin),lub(nghbrs(i,2),2)-(cntrPt(1,2)-fSizeXmin),lub(nghbrs(i,2),6),trckText,'Color','red')
        else
            nghbrs(i,2) = 0;
        end
    end
